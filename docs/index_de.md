---
title: AEON Datenübernahme
identifier: intranda_workflow_aeon_process_creation
description: Dieses Workflow Plugin ermöglicht die Erstellung von Goobi Vorgängen auf der Grundlage einer ersten AEON-Anfrage. Es ermöglicht unter anderem eine manuelle Auswahl von Elementen aus der AEON-Antwort.
published: true
keywords:
    - Goobi workflow
    - Plugin
...
![Anzeige der Daten nach Ausführung einer Suche](screen4_de.png)
...
![Ausgeklappte Boxen mit weiteren angezeigten Feldern](screen5_de.png)
...
![Zweite Seite mit der Möglichkeit einen Laufzettel herunterzuladen](screen6_de.png)
...
------------------- | -----------------------------------------------------
`materialTypeField` | Name des AEON-Feldes, das den Materialtyp enthält. Sein Wert wird mit dem Attribut `type` der Felder in `<properties>` verglichen und entscheidet damit, welche Eigenschaften angezeigt und validiert werden. Enthält der AEON-Record keinen Materialtyp, wird ein Hinweis angezeigt und die Anfrage fortgesetzt: es gilt dann kein auf einen Materialtyp beschränktes Feld, sodass die erzeugten Vorgänge allein die unbeschränkten Eigenschaften tragen. Standardwert: `customFieldValues/MaterialType`.
